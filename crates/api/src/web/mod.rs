...
pub async fn get_current_user(
    ...
    if let Some(auth_user) = auth {
        match state.user_repo.find_by_id(auth_user.id).await {
            Ok(Some(user)) => Some(UserDto::from(user)),
            _ => None,
        }
    } else {
        None
...
pub fn web_router() -> Router<AppState> {
    Router::new()
        ...
        .route("/listings/create", axum::routing::post(listing_create::listing_create_post_handler))
        ...
        .route("/users/me", get(profile::profile_me_handler))
