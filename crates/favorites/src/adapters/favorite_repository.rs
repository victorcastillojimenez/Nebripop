...
/// Fila de base de datos para la entidad Favorite (privada al adaptador).
...
#[async_trait]
impl FavoritePort for FavoriteRepository {
    async fn insert_favorite(
        ...
        match result {
            Some(row) => Ok((Favorite::from(row), false)), // Se insertó nuevo
            None => {
                // Ya existía — recuperarlo
                let existing = sqlx::query_as::<_, FavoriteRow>(
                ...
                .map_err(|e| {
                    tracing::error!(
                        "Database error in insert_favorite (fetch existing): {}",
                        e
                    );
                    FavoriteError::DatabaseError(e.to_string())
                })?;

                Ok((Favorite::from(existing), true)) // Ya existía
            }
        }
    }
